...
import { SFTPFormData, StepProps } from "../types/forms";
...
export const GeneratedJSON: React.FC<StepProps> = ({ formData, darkMode }) => {
  ...
  const generateConfig = (data: SFTPFormData) => {
    const config: any = {
      name: data.name,
      host: data.host,
      port: data.port || 22,
    ...
    if (data.showAdvancedOptions) {
      ...
      if (data.connectionTimeout)
        config.connectionTimeout = data.connectionTimeout;
      if (data.keepAliveInterval)
        config.keepAliveInterval = data.keepAliveInterval;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className={`text-sm ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
        Copy this configuration to your SFTP client's settings or save it as a
        JSON file.
      </p>
    </div>
  );
