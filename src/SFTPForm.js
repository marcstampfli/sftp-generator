      ...
      <div className={`w-full max-w-3xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-2xl rounded-xl p-10 transition-colors duration-300 ease-in-out`}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8">
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              className={`w-full bg-blue-600 text-white text-xl font-semibold p-4 rounded-lg hover:bg-blue-700 transition duration-300 focus:outline-none focus:ring-4 focus:ring-blue-300 flex items-center justify-center ${darkMode ? 'bg-blue-800 hover:bg-blue-900' : ''}`}
            >
        ...
        {generatedJSON && (
          <div className="mt-12" ref={generatedContentRef}>
            ...
            <div className="relative overflow-hidden rounded-lg shadow-lg">
              <div className={`p-6 border-l-4 ${darkMode ? 'border-blue-400 bg-gray-800 text-green-400' : 'border-blue-600 bg-gray-100 text-gray-800'}`} style={{ fontFamily: 'monospace', lineHeight: '1.5', position: 'relative' }}>
                <div className="absolute top-0 right-0 m-3 px-2 py-1 bg-blue-500 text-white text-xs rounded">JSON Output</div>
                <pre className="text-left pl-10 whitespace-pre overflow-x-auto">
                  {generatedJSON.split('\n').map((line, index) => (
                    <div key={index} className="flex">
                      <span className={`mr-4 text-gray-500 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{index + 1}</span>
                      <span>{line}</span>
                    </div>
                  ))}
                </pre>
            ...
            <div className="flex justify-between mt-6">
              <button
                ...
                className={`bg-green-500 text-white text-lg font-semibold p-4 rounded-lg hover:bg-green-600 transition duration-300 focus:outline-none focus:ring-4 focus:ring-green-300 flex items-center ${darkMode ? 'bg-green-700 hover:bg-green-800' : ''}`}
              >
                <FiCopy className="mr-2" /> Copy to Clipboard
              </button>
              <button
                onClick={handleDownload}
                className={`bg-indigo-600 text-white text-lg font-semibold p-4 rounded-lg hover:bg-indigo-700 transition duration-300 focus:outline-none focus:ring-4 focus:ring-indigo-300 flex items-center ${darkMode ? 'bg-indigo-700 hover:bg-indigo-800' : ''}`}
              >
